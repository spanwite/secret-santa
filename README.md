...
### Как работают сгенерированные ссылки

Каждая ссылка формируется на основе текущего домена и содержит всю необходимую информацию в параметрах URL:

1. В качестве основы используется `location.origin`
2. Добавляется путь к странице просмотра: `/view.html`
3. В поисковую строку добавляются параметры:
...
> **Примечание**
> Кодирование используется для того, чтобы организатор, рассылающий ссылки, случайно не увидел распределение пар, просто взглянув на URL.
