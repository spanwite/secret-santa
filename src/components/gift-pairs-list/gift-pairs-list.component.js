export class GiftPairsListComponent {
	name = 'gift-pairs-list';
	...
	bindEvents() {
		...
		document.addEventListener(this.giftPairsService.eventNames.update, this.render.bind(this));
	}
	...
	handleClick({ target }) {
		if (target.matches(this.selectors.itemCopyButton)) {
			...
			const { link } = this.giftPairsService.findByName(itemId);
			navigator.clipboard.writeText(link);
		}
	}

	render() {
		this.elements.root.innerHTML = this.giftPairsService.giftPairs
